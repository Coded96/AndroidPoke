...
class BulbasaurFragment : Fragment() {
    private var listener: OnBulbasaurFragmentActionListener? = null
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        val vista: View = inflater.inflate(R.layout.fragment_bulbasaur, container, false)

        if(vista != null){
            try{
                txtPokeName = vista.findViewById(R.id.txtBulbasaurName)
            }catch(ex:Exception){
    ...
    override fun onAttach(context: Context) {
        ...
        if (context is OnBulbasaurFragmentActionListener) {
            listener = context
        }
    }
    ...
    companion object {
        @JvmStatic
        fun newInstance(entrenador: Jugador?): BulbasaurFragment =
            BulbasaurFragment().apply {
                return try{
                    val fragment = BulbasaurFragment()
                    val args = Bundle()
    ...
    interface OnBulbasaurFragmentActionListener {
        fun onBulbasaurClickFragmentButton()
    }
}